...
component {
	...
	public string function render(any records, string params="") {
		...
		html.add('<div class="paging">');
		html.add('<ul>');
		html.add('<li class="paging_count first">Records #numberFormat(options.start)#-#numberFormat(options.end)# of #numberFormat(options.count)#</li>');
		
		if (options.page > 1) {
			html.add('<li class="paging_first"><a href="#buildLink(options.page_link, "page=1")#" title="First">First</a></li>');
		} 
		else {
			html.add('<li class="paging_first">First</li>');
		}
		
		if (options.page > 1) {
			html.add('<li class="paging_previous"><a href="#buildLink(options.page_link, "page=#options.page-1#")#" title="Previous">Previous</a></li>');
		} 
		else {
			html.add('<li class="paging_previous">Previous</li>');
		}

		if (options.page < options.pages) {
			html.add('<li class="paging_next"><a href="#buildLink(options.page_link, "page=#options.page+1#")#" title="Next">Next</a></li>');
		} 
		else {
			html.add('<li class="paging_next">Next</li>');
		}
		
		if (options.pages > 1) {
			html.add('<li class="paging_last"><a href="#buildLink(options.page_link, "page=#options.pages#")#" title="Last">Last</a></li>');
		} 
		else {
			html.add('<li class="paging_last">Last</li>');
		}
		
		if (options.pages <= 100) {
			
			html.add('<li class="paging_jump"><span>Page</span>');
			
			var onchange = "window.location='#jsStringFormat(buildLink(options.page_link, "page="))#'+this.value;";
			
			html.add('<select name="paging_page" onchange="#onchange#">');
			
			for (i=1; i <= options.pages; i++) {
				
				if (options.page == i) {
					html.add('<option value="#i#" title="#i#" selected="true">#i#</option>');
				}
				else {
					html.add('<option value="#i#" title="#i#">#i#</option>');
				}
		...
		html.add('<li class="paging_max"><span>Per Page</span>');
		
		var onchange = "window.location='#jsStringFormat(buildLink(options.max_link, "max="))#'+this.value;";
		
		html.add('<select name="paging_max" onchange="#onchange#">');
		...
		for (i=1; i <= arrayLen(max_options); i++) {
			...
			if (options.max == option) {
				html.add('<option value="#option#" title="#option#" selected="true">#option#</option>');
			}
			else {
				html.add('<option value="#option#" title="#option#">#option#</option>');
			}
