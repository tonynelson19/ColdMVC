component {
	 ...
	 */
	 public boolean function isMobile() {
		...
		var userAgent = getCGIScope().getValue("http_user_agent");

		return reFindNoCase("android.+mobile|avantgo|bada\/|blackberry|blazer|compal|elaine|fennec|hiptop|iemobile|ip(hone|od)|iris|kindle|lge |maemo|midp|mmp|opera m(ob|in)i|palm( os)?|phone|p(ixi|re)\/|plucker|pocket|psp|symbian|treo|up\.(browser|link)|vodafone|wap|windows (ce|phone)|xda|xiino", userAgent)
	...
	/**
	 * @actionHelper isSecure
	 * @viewHelper isSecure
	 */
	public boolean function isSecure() {

		var https = getCGIScope().getValue("https");
	...
	public string function getMethod() {
		...
		return getCGIScope().getValue("request_method");
	...
	private any function getCGIScope() {

		return coldmvc.framework.getBean("cgiScope");
